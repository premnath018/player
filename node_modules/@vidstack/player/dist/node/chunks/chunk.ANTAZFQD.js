...
var VideoFullscreenController = class extends FullscreenController {
  ...
  _addFullscreenChangeEventListener(handler) {
    ...
    if (this.isSupportedOnSafari) {
      if (false) {
        (_a = this._logger) == null ? void 0 : _a.debug("adding `vds-video-presentation-change` listener");
      }
      return listen(
        this._host,
...
var VideoPresentationController = class {
  constructor(_host) {
    ...
    _host.firstUpdated = (changedProperties) => {
      firstUpdated == null ? void 0 : firstUpdated.call(_host, changedProperties);
  ...
  get presentationMode() {
    var _a;
    return (_a = this._host.videoElement) == null ? void 0 : _a.webkitPresentationMode;
  }
  get isInlineMode() {
  ...
  get isSupported() {
    var _a;
    return isFunction((_a = this._host.videoElement) == null ? void 0 : _a.webkitSetPresentationMode);
  }
  setPresentationMode(mode) {
    var _a, _b;
    (_b = (_a = this._host.videoElement) == null ? void 0 : _a.webkitSetPresentationMode) == null ? void 0 : _b.call(_a, mode);
  }
  _addPresentationModeChangeEventListener() {
    if (!this.isSupported || isNil(this._host.videoElement))
      return noop2;
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.debug("adding `webkitpresentationmodechanged` listener");
    }
    return listen2(
  ...
  _handlePresentationModeChange(event) {
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.infoGroup("presentation mode change").labelledLog("Event", event).dispatch();
    }
...
var VideoElement = class extends Html5MediaElement {
  ...
  static get styles() {
    return [
      i`:host{display:inline-block;background-color:var(--vds-video-bg-color,#000)}:host([hidden]){display:none}::slotted(video:not([width])){width:var(--vds-video-width,100%)}::slotted(video:not([height])){height:var(--vds-video-height,auto)}`
    ];
  }
  connectedCallback() {
