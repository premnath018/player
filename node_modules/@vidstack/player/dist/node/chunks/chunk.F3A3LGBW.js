...
var Html5MediaElement = class extends MediaProviderElement {
  ...
  _cancelTimeUpdates() {
    if (isNumber(this._timeRAF))
      (typeof window !== "undefined" ? window : null)?.cancelAnimationFrame(this._timeRAF);
    this._timeRAF = void 0;
  }
  _requestTimeUpdates() {
  ...
  _requestTimeUpdate() {
    var _a;
    const newTime = ((_a = this.mediaElement) == null ? void 0 : _a.currentTime) ?? 0;
    if (this.state.currentTime !== newTime) {
      this._updateCurrentTime(newTime);
    }
    this._timeRAF = (typeof window !== "undefined" ? window : null)?.requestAnimationFrame(() => {
      if (isUndefined(this._timeRAF))
        return;
      this._requestTimeUpdate();
    });
  ...
  _updateCurrentTime(newTime, triggerEvent) {
    ...
    this.dispatchEvent(
      vdsEvent("vds-time-update", {
        detail: {
          currentTime: Math.min(newTime, ((_a = this.mediaElement) == null ? void 0 : _a.duration) ?? 0),
  ...
  _findSlottedMediaElement() {
    ...
    const tagName = (mediaElement == null ? void 0 : mediaElement.tagName) ?? "";
  ...
  _handleMediaElementConnect() {
    (typeof window !== "undefined" ? window : null)?.requestAnimationFrame(() => {
      ...
      if (false) {
        (_a = this._logger) == null ? void 0 : _a.infoGroup("Media element connected").labelledLog("Media Element", mediaEl).dispatch();
      }
  ...
  _handleMediaElementDisconnect() {
    ...
    if (this._hasMediaElementConnected && false) {
      (_a = this._logger) == null ? void 0 : _a.infoGroup("Media element disconnected").labelledLog("Media Element", this.mediaElement).dispatch();
    }
    this._hasMediaElementConnected = false;
  }
  startLoadingMedia() {
  ...
  _getMediaSources() {
    var _a, _b;
    const resources = [
      (_a = this.mediaElement) == null ? void 0 : _a.src,
      ...Array.from(((_b = this.mediaElement) == null ? void 0 : _b.querySelectorAll("source")) ?? []).map(
        (source) => source.src
      )
    ].filter(Boolean);
  ...
  _attachMediaEventListeners() {
    ...
    keysOf(mediaEventListeners).forEach((type) => {
      ...
      const off = listen(this.mediaElement, type, async (event) => {
        if (false) {
          (_a2 = this._logger) == null ? void 0 : _a2.debugGroup(`\u{1F4FA} fired \`${event.type}\``).labelledLog("Event", event).labelledLog("State", { ...this.state }).dispatch();
        }
        await handler(event);
      });
    ...
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.debug("attached event listeners");
    }
  }
  _handleAbort(event) {
  ...
  _getPaused() {
    var _a;
    return ((_a = this.mediaElement) == null ? void 0 : _a.paused) ?? true;
  }
  _getVolume() {
    var _a;
    return ((_a = this.mediaElement) == null ? void 0 : _a.volume) ?? 1;
  }
  ...
  _getCurrentTime() {
    var _a;
    return ((_a = this.mediaElement) == null ? void 0 : _a.currentTime) ?? 0;
  }
  ...
  _getMuted() {
    var _a;
    return ((_a = this.mediaElement) == null ? void 0 : _a.muted) ?? false;
  }
  _setMuted(isMuted) {
  ...
  async play() {
    var _b;
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.info("attempting to play...");
    }
    try {
      this._throwIfNotReadyForPlayback();
      await this._resetPlaybackIfEnded();
      return (_b = this.mediaElement) == null ? void 0 : _b.play();
    } catch (error) {
  ...
  async pause() {
    var _b;
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.info("attempting to pause...");
    }
    return (_b = this.mediaElement) == null ? void 0 : _b.pause();
  }
  _getMediaType() {
