...
async function importHlsConstructor(importer, callbacks = {}) {
  ...
  if (!isFunction(importer)) {
    (_b = callbacks.onLoaded) == null ? void 0 : _b.call(callbacks, importer);
    return importer;
  }
  const cacheKey = String(importer);
  if (HLS_CTOR_CACHE.has(cacheKey)) {
    const hlsCtor = HLS_CTOR_CACHE.get(cacheKey);
    (_c = callbacks.onLoaded) == null ? void 0 : _c.call(callbacks, hlsCtor);
    return hlsCtor;
  }
  try {
    const hlsCtor = (_d = await importer()) == null ? void 0 : _d.default;
    if (hlsCtor && !!hlsCtor.isSupported) {
      (_e = callbacks.onLoaded) == null ? void 0 : _e.call(callbacks, hlsCtor);
      HLS_CTOR_CACHE.set(cacheKey, hlsCtor);
    } else {
      throw Error(
        false ? "[vds]: Failed importing `hls.js`. Dynamic import returned invalid constructor." : ""
      );
    }
    return hlsCtor;
  } catch (err) {
    (_f = callbacks.onLoadError) == null ? void 0 : _f.call(callbacks, err);
  }
  return void 0;
}
async function loadHlsConstructorScript(src, callbacks = {}) {
  ...
  try {
    ...
    if (!isFunction((typeof window !== "undefined" ? window : null)?.Hls)) {
      throw Error(
        false ? "[vds]: Failed loading `hls.js`. Could not find a valid Hls constructor on window" : ""
      );
    }
    const ctor = (typeof window !== "undefined" ? window : null)?.Hls;
...
var HlsElement = class extends VideoElement {
  constructor() {
    ...
    this.hlsConfig = {};
    this.hlsLibrary = false ? HLS_CDN_SRC_DEV : HLS_CDN_SRC_PROD;
    this._currentHlsSrc = "";
  ...
  get isHlsSupported() {
    var _a;
    return ((_a = this.Hls) == null ? void 0 : _a.isSupported()) ?? isHlsjsSupported();
  }
  get isHlsStream() {
  ...
  _preconnectToHlsLibDownload() {
    ...
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.infoGroup("Preconnect to `hls.js` download").labelledLog("URL", this.hlsLibrary).dispatch();
    }
  ...
  async _buildHlsEngine(forceRebuild = false) {
    ...
    if (isNil(this.videoElement) && !forceRebuild && !isUndefined2(this.hlsEngine)) {
      (_a = this._logger) == null ? void 0 : _a.infoGroup("\u{1F3D7}\uFE0F Could not build HLS engine").labelledLog("Video Element", this.videoElement).labelledLog("HLS Engine", this.hlsEngine).labelledLog("Force Rebuild Flag", forceRebuild).dispatch();
      return;
    }
    if (false) {
      (_b = this._logger) == null ? void 0 : _b.info("\u{1F3D7}\uFE0F Building HLS engine");
    }
    ...
    const callbacks = {
      onLoadStart: () => {
        if (false) {
          (_a2 = this._logger) == null ? void 0 : _a2.infoGroup("Starting to load `hls.js`").labelledLog("URL", this.hlsLibrary).dispatch();
        }
        this.dispatchEvent(vdsEvent("vds-hls-lib-load-start"));
      },
      onLoaded: (HlsConstructor) => {
        if (false) {
          (_a2 = this._logger) == null ? void 0 : _a2.infoGroup("Loaded `hls.js`").labelledLog("URL", this.hlsLibrary).labelledLog("Library", HlsConstructor).dispatch();
        }
        this.dispatchEvent(vdsEvent("vds-hls-lib-loaded", { detail: HlsConstructor }));
      },
      onLoadError: (err) => {
        if (false) {
          (_a2 = this._logger) == null ? void 0 : _a2.errorGroup("Failed to load `hls.js`").labelledLog("Lib Loader", this.hlsLibrary).labelledLog("Error", err).dispatch();
        }
    ...
    if (!((_d = (_c = this.Hls) == null ? void 0 : _c.isSupported) == null ? void 0 : _d.call(_c))) {
      const message = "[vds]: `hls.js` is not supported in this environment";
      if (false) {
        (_e = this._logger) == null ? void 0 : _e.error(message);
      }
    ...
    if (false) {
      (_f = this._logger) == null ? void 0 : _f.infoGroup("\u{1F3D7}\uFE0F HLS engine built").labelledLog("Video Element", this.videoElement).labelledLog("HLS Engine", this.hlsEngine).dispatch();
    }
  ...
  _attachHlsEngine() {
    ...
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.infoGroup("\u{1F3D7}\uFE0F Attached HLS engine").labelledLog("Video Element", this.videoElement).labelledLog("HLS Engine", this._hlsEngine).dispatch();
    }
  }
  _detachHlsEngine() {
    var _a;
    if (!this.isHlsEngineAttached)
      return;
    (_a = this.hlsEngine) == null ? void 0 : _a.detachMedia();
    this._isHlsEngineAttached = false;
    this._currentHlsSrc = "";
    if (false) {
      (_b = this._logger) == null ? void 0 : _b.infoGroup("\u{1F3D7}\uFE0F Detached HLS engine").labelledLog("Video Element", this.videoElement).dispatch();
    }
  }
  _loadSrcOnHlsEngine(src) {
    ...
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.infoGroup(`\u{1F4FC} Loading Source`).labelledLog("Src", src).labelledLog("Video Element", this.videoElement).labelledLog("HLS Engine", this._hlsEngine).dispatch();
    }
  ...
  _destroyHlsEngine() {
    var _a;
    (_a = this._hlsEngine) == null ? void 0 : _a.destroy();
    this._currentHlsSrc = "";
    this._hlsEngine = void 0;
    this._isHlsEngineAttached = false;
    if (false) {
      (_b = this._logger) == null ? void 0 : _b.info("\u{1F3D7}\uFE0F Destroyed HLS engine");
    }
  }
  ...
  async _handleHlsSrcChange(src) {
    ...
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.debug(`\u{1F4FC} Detected HLS source change \`${this.state.src}\``);
    }
  ...
  _listenToHlsEngine() {
    ...
    this._hlsEventListeners.forEach(({ type, listener, options }) => {
      var _a;
      (_a = this.hlsEngine) == null ? void 0 : _a[(options == null ? void 0 : options.once) ? "once" : "on"](type, listener, options == null ? void 0 : options.context);
    });
  ...
  _handleHlsError(eventType, data) {
    if (isUndefined2(this.Hls))
      return;
    if (false) {
      (_a = this._logger) == null ? void 0 : _a.errorGroup(`HLS error \`${eventType}\``).labelledLog("Video Element", this.videoElement).labelledLog("HLS Engine", this._hlsEngine).labelledLog("Event Type", eventType).labelledLog("Data", data).labelledLog("Src", this.state.src).labelledLog("State", { ...this.state }).dispatch();
    }
    if (data.fatal) {
      switch (data.type) {
        case "networkError":
  ...
  _handleHlsNetworkError() {
    var _a;
    (_a = this.hlsEngine) == null ? void 0 : _a.startLoad();
  }
  _handleHlsMediaError() {
    var _a;
    (_a = this.hlsEngine) == null ? void 0 : _a.recoverMediaError();
  }
  ...
  addEventListener(type, listener, options) {
    ...
    if (isHlsEventType(type)) {
      ...
      if (!hasEventListener) {
        ...
        (_a = this.hlsEngine) == null ? void 0 : _a[(options == null ? void 0 : options.once) ? "once" : "on"](hlsEventType, listener, options == null ? void 0 : options.context);
      }
      return;
    }
  ...
  removeEventListener(type, listener, options) {
    ...
    if (isHlsEventType(type)) {
      ...
      (_a = this.hlsEngine) == null ? void 0 : _a.off(hlsEventType, listener, options == null ? void 0 : options.context, options == null ? void 0 : options.once);
      return;
    }
...
__decorateClass([
  e({ type: Object, attribute: "hls-config" })
], HlsElement.prototype, "hlsConfig", 2);
__decorateClass([
  e({ attribute: "hls-library" })
